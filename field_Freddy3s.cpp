...
double field::f_x(double xx, double yy)
{
	...
	return -0.0006*(xx*xx*xx*xx*xx) - f_Gaussian(xx, A, 1.5, 0.5)*pe_Gaussian(yy, A, 1.8, 0.5)  - f_Gaussian(xx, A, 0.5, width)*pe_Gaussian(yy, A, -0.5, width)- f_Gaussian(xx, A, -1.5, width)*pe_Gaussian(yy, A, 1.5, width);
}
double field::f_x(double xx, double yy, vector<double> x0_bias)
{
    ...
    return -0.0006*(xx*xx*xx*xx*xx) - f_Gaussian(xx, A, 1.5, 0.5)*pe_Gaussian(yy, A, 1.8, 0.5)  - f_Gaussian(xx, A, 0.5, width)*pe_Gaussian(yy, A, -0.5, width)- f_Gaussian(xx, A, -1.5, width)*pe_Gaussian(yy, A, 1.5, width) + f_bias(xx, h_bias, x0_bias, w_bias);
...
double field::f_x(double xx, double yy, vector<double> x0_bias, vector<double> h0_bias)
{
	...
	return -0.0006*(xx*xx*xx*xx*xx) - f_Gaussian(xx, A, 1.5, 0.5)*pe_Gaussian(yy, A, 1.8, 0.5)  - f_Gaussian(xx, A, 0.5, width)*pe_Gaussian(yy, A, -0.5, width)- f_Gaussian(xx, A, -1.5, width)*pe_Gaussian(yy, A, 1.5, width) + f_bias(xx, h_bias, x0_bias, w_bias, delta_beta, h0_bias);
...
double field::f_x(double xx, double yy, vector<double> x0_bias, vector<double> y0_bias, vector<double> h0_bias)
{
	...
	return -0.0006*(xx*xx*xx*xx*xx) - f_Gaussian(xx, A, 1.5, 0.5)*pe_Gaussian(yy, A, 1.8, 0.5)  - f_Gaussian(xx, A, 0.5, width)*pe_Gaussian(yy, A, -0.5, width)- f_Gaussian(xx, A, -1.5, width)*pe_Gaussian(yy, A, 1.5, width) + coeff_x*f_bias(xx, yy, h_bias, x0_bias, y0_bias, w_bias, delta_beta, h0_bias, coeff_x, coeff_y);
...
{	
	...
	return -0.0006*(yy*yy*yy*yy*yy) - pe_Gaussian(xx, A, 1.5, 0.5)*f_Gaussian(yy, A, 1.8, 0.5) - pe_Gaussian(xx, A, 0.5, width)*f_Gaussian(yy, A, -0.5, width)- pe_Gaussian(xx, A, -1.5, width)*f_Gaussian(yy, A, 1.5, width);
}
...
double field::f_y(double xx, double yy, vector<double> y0_bias)
{
    ...
    return -0.0006*(yy*yy*yy*yy*yy) - pe_Gaussian(xx, A, 1.5, 0.5)*f_Gaussian(yy, A, 1.8, 0.5) - pe_Gaussian(xx, A, 0.5, width)*f_Gaussian(yy, A, -0.5, width)- pe_Gaussian(xx, A, -1.5, width)*f_Gaussian(yy, A, 1.5, width) + f_bias(yy, h_bias, y0_bias, w_bias);
...
double field::f_y(double xx, double yy, vector<double> y0_bias, vector<double> h0_bias)
{
    ...
    return -0.0006*(yy*yy*yy*yy*yy) - pe_Gaussian(xx, A, 1.5, 0.5)*f_Gaussian(yy, A, 1.8, 0.5) - pe_Gaussian(xx, A, 0.5, width)*f_Gaussian(yy, A, -0.5, width)- pe_Gaussian(xx, A, -1.5, width)*f_Gaussian(yy, A, 1.5, width) + f_bias(yy, h_bias, y0_bias, w_bias, delta_beta, h0_bias);
...
double field::f_y(double xx, double yy, vector<double> x0_bias, vector<double> y0_bias, vector<double> h0_bias)
{
    ...
    return -0.0006*(yy*yy*yy*yy*yy) - pe_Gaussian(xx, A, 1.5, 0.5)*f_Gaussian(yy, A, 1.8, 0.5) - pe_Gaussian(xx, A, 0.5, width)*f_Gaussian(yy, A, -0.5, width)- pe_Gaussian(xx, A, -1.5, width)*f_Gaussian(yy, A, 1.5, width) + coeff_y*f_bias(xx, yy, h_bias, x0_bias, y0_bias, w_bias, delta_beta, h0_bias, coeff_x, coeff_y);
