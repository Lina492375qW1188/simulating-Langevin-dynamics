...
input::input()
{
	...
	para_d = read_para<double>(f->inscript[9], "Potential_A", 2);
	A1= para_d[0];
	A2 = para_d[1];

	para_d = read_para<double>(f->inscript[10], "Potential_B", 3);
	B1 = para_d[0];
	B2 = para_d[1];
	B3 = para_d[2];
	
	para_d = read_para<double>(f->inscript[11], "Potential_C", 2);
	C1 = para_d[0];
	C2 = para_d[1];
	...
	j_bias = para_i[2]; // stride, so it needs to be integer.
