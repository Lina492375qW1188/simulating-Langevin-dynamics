...
double field::f_x(double xx, double yy)
{
	...
	- f_Gaussian(xx, A, -4.0, width)*pe_Gaussian(yy, A, 1.5, width)  \
	- f_Gaussian(xx, A, -1.0, width)*pe_Gaussian(yy, A, -3.0, width)  \
	- f_Gaussian(xx, A, 1.0, width)*pe_Gaussian(yy, A, 3.0, width) \
	- f_Gaussian(xx, A, 4.0, width)*pe_Gaussian(yy, A, -1.5, width) \
...
{	
	...
	- pe_Gaussian(xx, A, -4.0, width)*f_Gaussian(yy, A, 1.5, width) \
	- pe_Gaussian(xx, A, -1.0, width)*f_Gaussian(yy, A, -3.0, width) \
	- pe_Gaussian(xx, A, 1.0, width)*f_Gaussian(yy, A, 3.0, width) \
	- pe_Gaussian(xx, A, 4.0, width)*f_Gaussian(yy, A, -1.5, width) \
...
double field::f_y(double xx, double yy, vector<double> y0_bias)
{
    ...
    return -0.0006*(xx*xx*xx*xx*xx) - pe_Gaussian(xx, A, 2.0, width)*f_Gaussian(yy, A, -1.155, width) - pe_Gaussian(xx, A, 0.0, width)*f_Gaussian(yy, A, 2.309, width)- pe_Gaussian(xx, A, -2.0, width)*f_Gaussian(yy, A, -1.155, width) + f_bias(yy, h_bias, y0_bias, w_bias);
...
double field::f_y(double xx, double yy, vector<double> y0_bias, vector<double> h0_bias)
{
    ...
    return -0.0006*(xx*xx*xx*xx*xx) - pe_Gaussian(xx, A, 2.0, width)*f_Gaussian(yy, A, -1.155, width) - pe_Gaussian(xx, A, 0.0, width)*f_Gaussian(yy, A, 2.309, width)- pe_Gaussian(xx, A, -2.0, width)*f_Gaussian(yy, A, -1.155, width) + f_bias(yy, h_bias, y0_bias, w_bias, delta_beta, h0_bias);
...
double field::f_y(double xx, double yy, vector<double> x0_bias, vector<double> y0_bias, vector<double> h0_bias)
{
    ...
    return -0.0006*(xx*xx*xx*xx*xx) - pe_Gaussian(xx, A, 2.0, width)*f_Gaussian(yy, A, -1.155, width) - pe_Gaussian(xx, A, 0.0, width)*f_Gaussian(yy, A, 2.309, width)- pe_Gaussian(xx, A, -2.0, width)*f_Gaussian(yy, A, -1.155, width) + coeff_y*f_bias(xx, yy, h_bias, x0_bias, y0_bias, w_bias, delta_beta, h0_bias, coeff_x, coeff_y);
