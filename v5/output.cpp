...
output::output()
{
    ...
    // Force:
	epsilon = roundstr(_inp->epsilon, 1);
    sigma = roundstr(_inp->sigma, 1);
	...
	string LJ = "epsln"+epsilon+"sgma"+sigma; // Lennard-Jones
	string SG = "A"+A+"x0"+x0+"w"+width; // Shifted Gaussian
	string DW = "B"+B+"a"+a; // Double Well
	
	myfile = new ofstream[num_particle];
	
	for(int i=0; i<num_particle; i++){	
		string file_pos = "xvyw"+to_string(i+1) + Verlet + LJ + SG + DW + Environment + ".txt";	
